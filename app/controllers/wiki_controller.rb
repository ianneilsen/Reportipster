...
class WikiController < ApplicationController
  ...
  caches_action :show, :published, :authors, :print, :recently_revised, :list, :file_list, :source,
        :history, :revision, :atom_with_content, :atom_with_headlines, :if => Proc.new { |c| c.send(:do_caching?) }
  cache_sweeper :revision_sweeper

  layout 'default', :except => [:atom_with_content, :atom_with_headlines, :atom, :source, :publishemail, :export_html]
...
<head>
  ...
  <style type="text/css">
    h1#pageName, div.info, .newWikiWord a, a.existingWikiWord, .newWikiWord a:hover, [actiontype="toggle"]:hover, #TextileHelp h3 { 
      color: ##{@web ? @web.color : "800"};
    }
    a:visited.existingWikiWord {
      color: #{darken(@web ? @web.color : "800")};
    }   
  </style>
...
#  end
  ...
  def recently_revised
    ...
    @pages_by_revision.each do |page| 
      day = Date.new(page.created_at.year, page.created_at.month, page.created_at.day)
...
#  end
  ...
  def save
    ...
    author_name = 'QEBrisbane' if author_name =~ /^\s*$/
...
#  end
  ...
  def export_page_to_tex(file_path)
    if @web.markup == :markdown || @web.markup == :markdownPNG
  ...
  def render_atom(hide_description = false, limit = 30)
    @pages_by_revision = @web.select.by_created.first(limit)
    #@pages_by_days = @pages_in_category.by_revision.first(limit)
