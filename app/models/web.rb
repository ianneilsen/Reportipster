...
class Web < ActiveRecord::Base
  ...
  def last_5_weeks
    pages.all(:select => "pages.id", :order => "pages.name").reverse
  end
  ...
  def create_files_directory
    ...
    begin
      ...
      raise "We could not create a directory to store uploaded files. " +
            "Please make sure that you or this app is allowed to create directory " +
